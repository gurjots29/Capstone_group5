	...
	*/
	...
	Tree.prototype.buildTree = function (nodes, level) {
		...
		$.each(nodes, function addNodes(id, node) {
			...
			else if (node.textBlack){
				treeItem
					.append($(_this.template.negrita)
						.append(node.textBlack)
					)
			...
			// Add button tags for fathers as badges
			if (_this.options.showTags && node.buttonTags) {
				$.each(node.buttonTags, function addButtonTags(id, btnTag) {
					treeItem
						.append($(_this.template.badge)
			...
			if (_this.options.showTags && node.buttonTagsHijo) {
				$.each(node.buttonTagsHijo, function addButtonTags(id, btnTagChild) {
					treeItem
						.append($(_this.template.badge)
							.append($(_this.template.buttonDelete).attr({id: "cd_"+btnTagChild.id})
							)
						)
						.append($(_this.template.badge)
							.append($(_this.template.buttonEdit).attr({id: "ce_"+btnTagChild.id, id_padre: +btnTagChild.id_padre})
							)
						)
				});
			}
